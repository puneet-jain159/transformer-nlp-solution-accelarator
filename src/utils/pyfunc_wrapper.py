...
def load_model_tokenizer(model_uri, dst_path=None):
    ...
    flavor_conf = _get_flavor_configuration(local_model_path, "python_function")
    ...
    if conf is None:
        raise ValueError(
            "Conf file used to train the model the not found please log it in the artifact"
        )

    model = get_model_from_configuration(
        conf, os.path.join(local_model_path, flavor_conf.get("data"))
    )
    tokenizer = AutoTokenizer.from_pretrained(
        os.path.join(local_model_path, flavor_conf.get("data"))
    )
