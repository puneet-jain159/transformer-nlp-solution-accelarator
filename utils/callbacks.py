...
class MLflowCallback(TrainerCallback):
    ...
    def setup(self, args, state, model):
        ...
        self._create_model = os.getenv("CREATE_MFLOW_MODEL", "FALSE").upper() in ENV_VARS_TRUE_VALUES
        self._run_id = os.getenv("MLFLOW_RUN_ID", None)
        self._tracking_uri = os.getenv("MLFLOW_TRACKING_URI", None)
        if self._tracking_uri:
            print("setting_tracking_uri")
        ...
        if state.is_world_process_zero:
            if self._ml_flow.active_run() is None or self._nested_run or self._run_id:
                ...
                logger.info(f"MLflow run started with run_id={self._ml_flow.active_run().info.run_id}")
        ...
        if self._initialized and state.is_world_process_zero:
            ...
            if self._create_model:
                ...
                self._ml_flow.pyfunc.log_model("mlflow_model", 
                        ...
                        conda_env=model_env,
                        code_path=[os.path.abspath('utils/custom_func.py')],
